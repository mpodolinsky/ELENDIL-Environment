...
class GridWorldEnvIntrinsic(gym.Env):
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        self.visit_counts[tuple(self._agent_location)] += 1
    ...
    def step(self, action):
        ...
        # reward = 1.0 / (self.visit_counts[tuple(self._agent_location)])**2
        if self.visit_counts[tuple(self._agent_location)] == 0:
            reward = 1.0
        else: reward = -0.2
...
if __name__ == "__main__":
    env = GridWorldEnvIntrinsic(render_mode="human", size=15)
    ...
    while not done:
        ...
        print(f"Action: {action}, Observation: {obs}, Reward: {reward:.2f}, Terminated: {terminated}, Info: {info}")
